...
/**
 * Core class used to implement an author widget.
 *
 * @see WP_Widget
 */
class Flextension_Author_Widget extends WP_Widget {
	...
	/**
	 * Sets up a new Author widget instance.
	 */
	public function __construct() {
		...
		$this->default_settings = array(
			'title'      => esc_html__( 'Author', 'flextension' ),
			'author_id'  => 0,
			'followers'  => false,
			'avatar'     => true,
			'cover'      => false,
		...
		$widget_options = array(
			'classname'                   => 'flext-widget-author',
			'description'                 => esc_html__( 'Displays the author box with image and bio.', 'flextension' ),
			'customize_selective_refresh' => true,
		);

		parent::__construct( 'flext-author', esc_html__( 'Flextension Author', 'flextension' ), $widget_options );
	}

	/**
	 * Outputs the content for the current Author widget instance.
	 *
	 * @param array $args     Display arguments including 'before_title', 'after_title',
	 *                        'before_widget', and 'after_widget'.
	 * @param array $instance Settings for the current Author widget instance.
	 */
	public function widget( $args, $instance ) {
		...
		echo flextension_author_widget( $instance ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped

		echo $args['after_widget']; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	}

	/**
	 * Handles updating the settings for the current Author widget instance.
	 *
	...
	/**
	 * Outputs the settings form for the Author widget.
	 *
	...
	public function form( $instance ) {
		...
		<!-- Author -->
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'author_id' ) ); ?>"><?php echo esc_html__( 'Author', 'flextension' ); ?></label>
			<select name="<?php echo esc_attr( $this->get_field_name( 'author_id' ) ); ?>" id="<?php echo esc_attr( $this->get_field_id( 'author_id' ) ); ?>" class="widefat">
				<option value=""><?php echo esc_html__( 'Select an author', 'flextension' ); ?></option>
				<?php
				$authors = flextension_author_get_authors();

				if ( ! empty( $authors ) ) {
					foreach ( $authors as $author ) {
						?>
							<option value="<?php echo esc_attr( $author->ID ); ?>" <?php selected( $instance['author_id'], $author->ID ); ?>><?php echo esc_html( $author->display_name ); ?></option>
						<?php
					}
				}
				?>
	...
	public function admin_enqueue_scripts( $hook ) {
		...
		if ( 'widgets.php' === $hook ) {
			...
			wp_enqueue_script( 'flextension-author-widget-editor' );
		}
...
/**
 * Registers an Author widget.
 */
function flextension_author_widget_register() {
	register_widget( 'Flextension_Author_Widget' );
}

add_action( 'widgets_init', 'flextension_author_widget_register' );
